define(["./eos-launch.js"],function(_eosLaunch){"use strict";class BloxRestore extends _eosLaunch.PolymerElement{static get template(){return _eosLaunch.html`
    ...
    `}static get properties(){return{eos:{type:Object},wasm:{type:String},abi:{type:String},keyProvider:{type:String},wasmfilename:{type:String}}}_deploy(){const name=this.shadowRoot.querySelector("#name").value,abi=JSON.parse(this.abi),wasm=this.wasm;console.log(wasm);this.eos.setcode(name,0,0,wasm);this.eos.setabi(name,abi)}}window.customElements.define("eos-deploy",EosDeploy)});